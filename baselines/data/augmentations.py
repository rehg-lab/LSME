...
def perspective(image, mask, uv):
    ...
    if p_type == 0:
        pts2 = np.float32([[0,0], [W,0],
                         [0-int(0.3*W),H], [W+int(0.3*W),H]])
    
    if p_type == 1
        pts2 = np.float32([[0,0], [W,0-int(0.3*H)],
                         [0,H], [W,H+int(0.3*H)]])
    
    if p_type == 2:
        pts2 = np.float32([[0,0-int(0.3*H)], [W,0],
                           [0,H+int(0.3*H)], [W,H]])
    
    if p_type == 3:
        pts2 = np.float32([[0-int(0.3*W),0], [W+int(0.3*W),0],
                           [0,H], [W,H]])
    '''
...
if __name__ == "__main__":
    ...
    def apply_image_transform(t, name, n):

        for i in range(n):
            img = cv2.imread("TMP/dev_data/img.png")[:,:,[2,1,0]]
            seg = cv2.imread("TMP/dev_data/seg.png")
            seg[seg==255]=1
            uv_ls = np.load("TMP/dev_data/uv1_ls.npy")
            ...
            for uv1 in uv_ls:
                ...
                ax2.plot(uv1[1], uv1[0], '.',markersize=2.5)

            fig.suptitle(name)
            fig.tight_layout()

            plt.savefig(f"{dest_dir}/{name}_{i:03d}.png")
            plt.close()

    def apply_spatial_transform(t, name, n):
        ...
        for i in range(n):
            ...
            aug_img, aug_seg, aug_uv_ls = t(img, seg, uv_ls.copy())
            aug_uv_ls = aug_uv_ls

            aug_img = np.array(aug_img)
            aug_seg = np.array(aug_seg)*255
            ...
            for uv1, aug_uv1 in zip(uv_ls, aug_uv_ls):
                ax1.plot(uv1[1], uv1[0], '.',markersize=2.5)
                ax2.plot(aug_uv1[1], aug_uv1[0], '.',markersize=2.5)
    ...
    param_dict = {
        "HorizontalFlip":[0.5],
        "VerticalFlip":[0.5],
        "BrightnessAug":[0.5, 0.4, 1.6],
        "HueAug":[0.5, -0.5, 0.5],
        "GammaAug":[0.5, 0.25, 1.75],
        "ContrastAug":[0.5, 0.4, 1.6],
        "SaturationAug":[0.5, 0.0, 2.0],
        "BlurAug":[1.0, [5,7], 0.1, 3],
        "RandomMaskingAug":[0.75, 0.5, "domain_randomized"],
        }

    out_str = json.dumps(param_dict)
    with open("data/augmentation_parameters/local_contrastive.json", "w") as f:
        f.write(out_str)

    hflip_aug = HorizontalFlip(*param_dict['HorizontalFlip'])
    vflip_aug = VerticalFlip(*param_dict['VerticalFlip'])
    brightness_aug = BrightnessAug(*param_dict['BrightnessAug'])
    hue_aug = HueAug(*param_dict['HueAug'])
    gamma_aug = GammaAug(*param_dict['GammaAug'])
    contrast_aug = ContrastAug(*param_dict['ContrastAug'])
    saturation_aug = SaturationAug(*param_dict['SaturationAug'])
    blur_aug = BlurAug(*param_dict['BlurAug'])
    random_masking_aug = RandomMaskingAug(*param_dict['RandomMaskingAug'])

    dest_dir = "data/augmentation_tests"

    if not os.path.exists(dest_dir):
        os.makedirs(dest_dir)

    composed_aug = ContrastiveAugmentation(param_dict)

    #apply_spatial_transform(composed_aug, "composed_aug", 20)

    apply_spatial_transform(random_masking_aug, "random_masking", 10)
    apply_spatial_transform(hflip_aug, "horizontal flip", 1)
    apply_spatial_transform(vflip_aug, "vertical flip", 1)

    apply_image_transform(brightness_aug, "brightness", 10)
    apply_image_transform(hue_aug, "hue", 10)
    apply_image_transform(gamma_aug, "gamma", 10)
    apply_image_transform(contrast_aug, "contrast", 10)
    apply_image_transform(saturation_aug, "saturation", 10)
    apply_image_transform(blur_aug, "blur", 10)
